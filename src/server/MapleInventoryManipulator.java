            ...
            for (int i : potentials) {
                if (i > 0) {
                    ...
                    if (pot != null && pot.skillID > 0) {
                        ss.put(SkillFactory.getSkill(PlayerStats.getSkillByJob(pot.skillID, c.getPlayer().getJob())), new SkillEntry((byte) 1, (byte) 0, -1));
                    }
                }
            }
        ...
        if (source.getState() >= 17) {
            ...
            for (int i : potentials) {
                if (i > 0) {
                    StructPotentialItem pot = ii.getPotentialInfo(i).get(ii.getReqLevel(source.getItemId()) / 10);
                    if (pot != null && pot.skillID > 0) {
                        ss.put(SkillFactory.getSkill(PlayerStats.getSkillByJob(pot.skillID, c.getPlayer().getJob())), new SkillEntry((byte) 0, (byte) 0, -1));
                    }
                }
            }
            c.getPlayer().changeSkillLevel_Skip(ss, true);
