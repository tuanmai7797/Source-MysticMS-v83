...
public class MapleCharacter extends AnimatedMapleMapObject implements Serializable, MapleCharacterLook {
    ...
    public final void handleBattleshipHP(int damage) {
        if (damage < 0) {
            ...
            if (effect != null && effect.getSourceId() == 5221006) {
                ...
                if (battleshipHP <= 0) {
                    battleshipHP = 0;
                    client.getSession().write(CField.skillCooldown(5221006, effect.getCooldown()));
    ...
    public void baseSkills() {
		...
		if (GameConstants.isDemon(job)) {
			final int[] ss1 =  {30011000, 30011001, 30010002, 30010185, 30010112, 30010111, 30010110, 30010022, 30011109, 30010186, 30010183, 30010184};
			for (int i : ss1) {
				skil = SkillFactory.getSkill(i);
				if (skil != null) {
					if (getSkillLevel(skil) <= 0) { // no total
			...
			final int[] ss2 =  {30011170, 30011169, 30011168, 30011167, 30010166};
			for (int i : ss2) {
				skil = SkillFactory.getSkill(i);
				if (skil != null) {
					if (getSkillLevel(skil) <= 0) { // no total
                ...
                if (tot >= needed) {
                    ...
                    if ((level >= 200 || (GameConstants.isKOC(job) && level >= 120)) && !isIntern()) {
                        ...
                        if (exp >= needed) {
                         ...
                         if (job == 0 & level > 11 & level < 12) {
            NPCScriptManager.getInstance().start(getClient(), 2050002);
        }
                              if (job == 2410 & level > 30 & level < 31) {
            NPCScriptManager.getInstance().start(getClient(), 2050002);
        }
                                   if (job == 2411 & level > 70 & level < 71) {
            NPCScriptManager.getInstance().start(getClient(), 1092006);
        }
                                        if (job == 2412 & level > 120 & level < 121) {
    ...
    public void levelUp() {
        ...
        if (level == 200) {
            ...
            sb.append(getName());
            sb.append(" has reached Level 200. Congratulate ");
             if (medal != null) { // Medal
                sb.append("<");
                sb.append(MapleItemInformationProvider.getInstance().getName(medal.getItemId()));
                sb.append("> ");
             ...
             sb.append(getName());
             sb.append(" on such an amazing achievement!");
            ...
            if (getJob() == 122 && !skillisCooling(1220013)) {
                ...
                if (getTotalSkillLevel(divine) > 0) {
                    ...
                    if (divineShield.makeChanceResult()) {
                        ...
                        client.getSession().write(CField.skillCooldown(1220013, divineShield.getCooldown()));
                ...
                if (getTotalSkillLevel(divine) > 0 && !skillisCooling(divine.getId())) {
                    ...
                    if (divineShield.makeChanceResult()) {
                        divineShield.applyTo(this);
                        client.getSession().write(CField.skillCooldown(divine.getId(), divineShield.getX()));
                ...
                if (getTotalSkillLevel(divine) > 0 && !skillisCooling(divine.getId()) && getBuffSource(MapleBuffStat.BEHOLDER) == 1321007) {
                    final MapleStatEffect divineShield = divine.getEffect(getTotalSkillLevel(divine));
                    if (divineShield.makeChanceResult()) {
                        client.getSession().write(CField.skillCooldown(divine.getId(), divineShield.getCooldown()));
